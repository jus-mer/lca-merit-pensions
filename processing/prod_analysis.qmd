...
db_sem <- db_full %>% 
  dplyr::select(just_pension, just_educ, just_healthcare, all_of(vars_m), age, sex, educ, income_4, pol) %>% 
  drop_na()

db_sem <- db_sem %>% 
  mutate(
    across(.cols = starts_with("just"),
           .fns = ~ as.numeric(.))
  )
...
db_sem$income_4 <- relevel(db_sem$income_4, ref = "Low")
...
model <- c('
  ...
  mjp ~ perc_merit + perc_nmerit + pref_merit + pref_nmerit +
                 age + educ + sex_female +
                 income_4Middle.low + income_4Middle.high + income_4High +
